...
- [x] **CARD-015**: The sync job's bulk download shall be bounded by an idle/read deadline that fails the run if the connection delivers no bytes for a bounded interval, and the job shall not impose a single whole-request HTTP timeout that also spans the subsequent ingest.
