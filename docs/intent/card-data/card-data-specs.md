...
- [x] **CARD-015**: The sync job shall bound the non-body phase of every bulk HTTP call — the manifest fetch and the wait for response headers on each bulk GET — with a response-header timeout, and shall bound each bulk download's body with an idle/read deadline that fails the run if the connection delivers no bytes for a bounded interval; neither bound shall be a single whole-request HTTP timeout that also spans the subsequent ingest.
