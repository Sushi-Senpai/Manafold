...
- [x] **ACCT-017**: If more than a fixed number of `/api/auth/login` or `/api/auth/register` requests arrive from one client IP within the rate-limit window, then the system shall respond `429` until the bucket refills, without consulting the database, keying the limiter on the first entry of the `TRUSTED_PROXY_COUNT`-long trusted suffix of `X-Forwarded-For` (default `1`, fail-safe) and falling back to the connection's remote address when the header is absent or the chain is shorter than `TRUSTED_PROXY_COUNT`, so a caller cannot reset its bucket by prepending a forged entry.
