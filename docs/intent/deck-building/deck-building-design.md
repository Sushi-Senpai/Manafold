...
| Action-menu shortcut binding | Bind Alt+1..4 to the row under the pointer **or** the row with keyboard focus-within, in an effect that does not depend on the menu being rendered; match on `event.code` (`Digit1`..`Digit4`); guard against a focused field | Bind them only while the menu is open; match on `event.key`; a global shortcut layer | The chords must keep working now that the menu no longer opens on hover, so they cannot hang off the menu. `event.code` is the physical key, sidestepping macOS Option+digit typography; pointer-or-focus scoping keeps them unambiguous without a full focus model, and the field guard stops them firing mid-search. |
